...
function AreaCommunity({ area, user }) {
	...
	return <section className="area-community"><div className="community-heading"><div><p className="eyebrow">Local conversation</p><h2>What are people seeing?</h2></div><span>{messages.length} posts</span></div><div className="community-feed">{messages.length ? messages.map((item) => <article className={`feed-message ${item.role}`} key={item.id}><div className="feed-meta"><strong>{item.author}</strong>{item.role === 'official' && <span>Official</span>}<small>{new Date(item.createdAt).toLocaleString()}</small></div><p>{item.message}</p></article>) : <p className="empty-feed">No local posts yet. Start the conversation.</p>}</div><form className="community-compose" onSubmit={post}><input value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Share a local observation or ask a question" aria-label="Area community message" /><button type="submit" aria-label="Post message" title="Post message">→</button></form></section>;
}
