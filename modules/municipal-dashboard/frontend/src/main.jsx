...
function MunicipalityReportRegister({ token }) {
	const [reports, setReports] = React.useState([]);
	React.useEffect(() => { fetch(`${api}/municipality/research`, { headers: { Authorization: `Bearer ${token}` } }).then((response) => response.ok ? response.json() : { data: { reports: [] } }).then((payload) => setReports(payload.data?.reports ?? [])); }, [token]);
