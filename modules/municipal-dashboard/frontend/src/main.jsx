...
function MunicipalityUpdatePanel({ token, onClose }) {
	...
	const [message, setMessage] = React.useState('');
	const [notice, setNotice] = React.useState('');
	React.useEffect(() => { fetch(`${api}/municipality/research`, { headers: { Authorization: `Bearer ${token}` } }).then((response) => response.json()).then((payload) => { setReports(payload.data.reports); setSelectedId(payload.data.reports[0]?.id ?? ''); }); }, [token]);
	...
	const publish = async (event) => { event.preventDefault(); const response = await fetch(`${api}/municipality/reports/${selectedId}/updates`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify({ status, reason, message }) }); const payload = await response.json(); if (!response.ok) { setNotice(payload.error ?? 'Update failed.'); return; } setNotice(`Update published to ${selectedId}.`); setMessage(''); };
...
function MunicipalityOperations({ token, onClose, onSignOut }) {
...
function AreaCommunity({ area, user }) {
	...
	const post = async (event) => { event.preventDefault(); if (!message.trim()) return; await fetch(`${api}/community/messages`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ area, name: user?.name, message }) }); setMessage(''); load(); };
	return <><button className={`chat-launcher ${open ? 'is-open' : ''}`} onClick={() => setOpen(!open)} aria-label={open ? 'Minimize local conversation' : 'Open local conversation'} title={open ? 'Minimize chat' : 'Open local chat'}><span className="chat-icon" aria-hidden="true"><i /><i /><i /></span><span>Local chat</span>{messages.length > 0 && <b>{messages.length}</b>}</button>{open && <section className="area-community floating-chat"><div className="community-heading"><div><p className="eyebrow">{area === 'all' ? 'All areas' : 'Local conversation'}</p><h2>What are people seeing?</h2></div><button className="chat-minimize" onClick={() => setOpen(false)} aria-label="Minimize chat">−</button></div><div className="chat-context">{area === 'all' ? 'Showing messages from all service areas' : 'People in this service area'}</div><div className="community-feed">{messages.length ? messages.map((item) => <article className={`feed-message ${item.role}`} key={item.id}><div className="feed-meta"><strong>{item.author}</strong>{item.role === 'official' && <span>Official</span>}<small>{new Date(item.createdAt).toLocaleString()}</small></div><p>{item.message}</p></article>) : <p className="empty-feed">No local posts yet. Start the conversation.</p>}</div><form className="community-compose" onSubmit={post}><input value={message} onChange={(event) => setMessage(event.target.value)} placeholder="Ask or share a local observation" aria-label="Area community message" /><button type="submit" aria-label="Post message" title="Post message">→</button></form></section>}</>;
}
...
function App() {
	...
	return <main className="shell">
		...
		{showMunicipality && <MunicipalityOperations token={municipalityToken} onClose={() => setShowMunicipality(false)} onSignOut={municipalitySignOut} />}
		...
		<AreaCommunity area={selectedArea} user={user} />
