...
Municipal report records include `loggedAt` (ISO 8601 UTC). Open reports expose elapsed time from `loggedAt`; resolved reports include `resolvedHours`. The research and broadcast routes require a municipality work-account bearer token from `/api/v1/municipality/auth/login`. Development credentials are configured with `MUNICIPALITY_WORK_EMAIL` and `MUNICIPALITY_WORK_PASSWORD`; replace this gate with Microsoft Entra work-account SSO in production. Broadcasts currently enter an in-memory queue and return the recipient count. Connect the queue to Azure Communication Services, SendGrid, or another SMTP provider for production delivery.
